...
export async function getAnimeByLetter({ q, page = 1, limit = 10 }) {
  const endpoint = `/anime?${new URLSearchParams({
    q,
    page,
    limit,
    order_by: "popularity",
  ...
  return cachedFetch(`/anime?${encodeURIComponent(endpoint)}`);
}
export async function getAnimeByAlphabet({ letter, page = 1, limit = 10 }) {
  const endpoint = `/anime?${new URLSearchParams({
    letter,
    page,
    limit,
